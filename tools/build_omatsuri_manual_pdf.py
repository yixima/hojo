...
def build(date_str, out=OUT):
    ...
    def decorate(canvas, docu):
        ...
        canvas.drawString(LM, BM - 8 * mm, '作成 2026年9月4日（金）／版 2.0')
    ...
    A(grid([
        ['書類', '押す場所'],
        ['送付状', '右上「代表理事　生島　儀尊」の右にある「印」の字に重ねて'],
        ['参加意向申出書（第1号様式）', '「代表者職氏名　代表理事　生島　儀尊」の行の右端「印」に重ねて'],
        ['誓約書（参考様式1）',
         '「代表者職氏名　代表理事　生島　儀尊」の<b>右</b>。'
         'この様式の押印マークは Word の特殊な部品でできており、'
         '<b>使うソフトによっては表示されないことがあります。</b>'
         '見当たらないときは氏名の右側（行の右端あたり）に押してください'],
        ['守秘義務誓約書（参考様式10-1）', '「代表者職・氏名　代表理事　生島　儀尊」の行の右端「印」に重ねて'],
    ], [62 * mm, W - 62 * mm]))
    A(Spacer(1, 5))
    ...
    A(grid([
        ...
        ['誓約書に押印の丸マークが見当たらない',
         'その様式の押印マークは Word の特殊な部品で、'
         '<b>使うソフトによっては表示されません。異常ではありません。</b>'
         '「代表者職氏名　代表理事　生島　儀尊」の右側に押してください。'],
        ['先方から知らない書類を求められた',
