...
def audit_ledger(rows):  # noqa: C901
    ...
    **書式違反は、エラーにならずに情報を失わせる。**だから明示的に点検する。
    ...
    # **列名そのものが壊れていないか。**2026-09-16、枝の統合で BOM（U+FEFF）が
    # ヘッダの先頭に入り、`r['初報日']` が KeyError になった。
    # **どの検査も捕まえず、板は平然と動いていた**（板は初報日を読まないため）。
    # 静かに壊れるものは、静かに壊れたまま残る。
    WANT = ['初報日', '案件名', '発注機関', '種別', '締切', '締切時刻', '締切種別',
            '締切確認日', '予定価格', '格付', '応募形態', '資格要否', '状態', 'URL']
    if rows:
        got = list(rows[0].keys())
        if got != WANT:
            ng.append((1, '台帳の列が想定と違う（BOMや列の増減を疑う）',
                       [g for g in got if g not in WANT] or got))
